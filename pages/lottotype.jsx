...
export default function NumberTypePage() {
    const [{ data: lottotypyeData }, getLottotype] = useAxios({ url: '/api/lottotype' })

    const [{ data: postData, error: errorMessage, loading: lottotypeLoading }, executeLottotype] = useAxios({ url: '/api/lottotypes', method: 'POST' }, { manual: true });
          ...
          <div className="d-flex align-items-center border-bottom py-2">
            <div className="table-responsive w-100">
              <Table className="table table-striped text-start align-middle  align-items-center table-hover ">
                ...
                <tbody>
                  {lottotypyeData?.map((lottotypye,index) => (
                     <tr key={index}>
                    <td>{lottotypye.name}</td>
